    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                ...
                <p class="text-2xl font-bold text-purple-600">${{ number_format($payroll->total_net, 2) }}</p>
            </div>
        </div>
        ...
        <div class="flex items-center space-x-3">
            @if($payroll->status == 'draft')
                <form action="{{ route('admin.payroll.calculate', $payroll) }}" method="POST" class="inline">
                    @csrf
            ...
            @if($payroll->status == 'approved')
                <form action="{{ route('admin.payroll.finalize', $payroll) }}" method="POST" class="inline">
                    @csrf
            ...
            @if($payroll->status == 'finalized')
                <form action="{{ route('admin.payroll.process-ach', $payroll) }}" method="POST" class="inline">
                    @csrf
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 text-sm font-semibold text-purple-600">${{ number_format($item->net_pay, 2) }}</td>
                    </tr>
                    @empty
                    <tr>
