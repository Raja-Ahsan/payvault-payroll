...
@section('page-title', 'Create New Payroll Run')
@section('page-description', 'Create a new payroll run for a company')

@section('content')
<div class="bg-white rounded-lg shadow-md p-6 max-w-4xl">
    <form action="{{ route('client.payroll.store') }}" method="POST">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="pay_period_type" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                    <option value="weekly" {{ old('pay_period_type') == 'weekly' ? 'selected' : '' }}>Weekly</option>
                    <option value="biweekly" {{ old('pay_period_type') == 'biweekly' ? 'selected' : '' }}>Biweekly</option>
                    <option value="semimonthly" {{ old('pay_period_type') == 'semimonthly' ? 'selected' : '' }}>Semi-Monthly</option>
                    <option value="monthly" {{ old('pay_period_type') == 'monthly' ? 'selected' : '' }}>Monthly</option>
                </select>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Pay Period Start *</label>
                <input type="date" name="pay_period_start" value="{{ old('pay_period_start') }}" required
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Pay Period End *</label>
                <input type="date" name="pay_period_end" value="{{ old('pay_period_end') }}" required
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
            </div>
        ...
        <div class="mt-6 flex items-center justify-end space-x-4">
            <a href="{{ route('client.payroll.index') }}" class="px-6 py-2 border rounded-lg text-gray-700 hover:bg-gray-50">
                Cancel
            </a>
