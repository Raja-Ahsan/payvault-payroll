...
@php
    ...
    $pd = $payrollItem->pay_date ? \Illuminate\Support\Carbon::parse($payrollItem->pay_date)->format('Y-m-d') : '';
@endphp

@section('title', 'Edit Payroll')
@section('page-title', 'Edit Payroll')
@section('page-description', 'Update payroll entry for '.$employee->name)
...
@section('content')
    <div class="bg-white rounded-lg shadow-md p-6"
        ...
        <div class="mb-6 pb-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-600 mt-1">401(k): <span class="font-medium">{{ number_format($k401Percent, 2) }}%</span> of gross. Deduction = gross × ({{ number_format($k401Percent, 2) }} ÷ 100).</p>
        </div>

        <form action="{{ route('client.employees.payroll.update', [$employee, $payrollItem]) }}" method="POST" id="payroll-form">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Pay date *</label>
                    <input type="date" name="pay_date" value="{{ old('pay_date', $pd) }}" required
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="regular_hours" id="regular_hours" value="{{ old('regular_hours', $payrollItem->regular_hours) }}" required
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="vacation_hours" id="vacation_hours" value="{{ old('vacation_hours', $payrollItem->vacation_hours) }}" required
                        class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Sick hours *</label>
                    <input type="number" step="0.01" name="sick_hours" id="sick_hours" value="{{ old('sick_hours', $payrollItem->sick_hours) }}" required
                        class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Holiday hours *</label>
                    <input type="number" step="0.01" name="holidays_hours" id="holidays_hours" value="{{ old('holidays_hours', $payrollItem->holidays_hours) }}" required
                        class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Personal hours *</label>
                    <input type="number" step="0.01" name="personal_hours" id="personal_hours" value="{{ old('personal_hours', $payrollItem->personal_hours) }}" required
                        class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Overtime hours *</label>
                    <input type="number" step="0.01" name="overtime_hours" id="overtime_hours" value="{{ old('overtime_hours', $payrollItem->overtime_hours) }}" required
                        class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                </div>
                ...
                <div class="md:col-span-2 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="fed_tax" id="fed_tax" value="{{ old('fed_tax', $payrollItem->fed_tax) }}" required
                                class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">State tax *</label>
                            <input type="number" step="0.01" name="state_tax" id="state_tax" value="{{ old('state_tax', $payrollItem->state_tax) }}" required
                                class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Local tax *</label>
                            <input type="number" step="0.01" name="local_tax" id="local_tax" value="{{ old('local_tax', $payrollItem->local_tax) }}" required
                                class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Social Security *</label>
                            <input type="number" step="0.01" name="social_security" id="social_security" value="{{ old('social_security', $payrollItem->social_security) }}" required
                                class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Medicare *</label>
                            <input type="number" step="0.01" name="medi_care" id="medi_care" value="{{ old('medi_care', $payrollItem->medi_care) }}" required
                                class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Insurance deduction *</label>
                            <input type="number" step="0.01" name="insurance_deduction" id="insurance_deduction" value="{{ old('insurance_deduction', $payrollItem->insurance_deduction ?? $defaultInsurance) }}" required
                                class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Other deductions *</label>
                            <input type="number" step="0.01" name="other_deductions" id="other_deductions" value="{{ old('other_deductions', $payrollItem->other_deductions ?? $defaultOther) }}" required
                                class="js-payroll-recalc w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                        </div>
                    </div>
                    ...
                    <div class="border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
                        <div class="px-4 py-3 bg-gray-100 border-b border-gray-200">
                            <span class="text-lg font-semibold text-gray-800">Deductions summary</span>
                        </div>
